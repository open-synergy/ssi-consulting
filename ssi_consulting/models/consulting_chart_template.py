...
class ConsultingChartTemplate(models.Model):
    ...
    def _compute_payload(self):
        for rec in self:
            ...
            datasource_id = getattr(rec.materialized_view_id, "superset_dataset_id", None) or "__DATASOURCE_ID__"
            ...
            for f in filters:
                ...
                if col and op:
                    adhoc_filters.append({
                        ...
                        "subject": col, "operator": op, "comparator": val
                    })
            ...
            if viz_type == "table":
            ...
            elif viz_type in ("echarts_timeseries_line","echarts_timeseries_area","echarts_timeseries_bar","mixed_timeseries"):
                form_data.update({
                    "metrics": metrics,
                ...
                group = [enc_color] if enc_color else ([enc_x] if enc_x else (group_by[:1] if group_by else []))
                form_data.update({
                    "groupby": group,
                ...
                if enc_x and enc_color and enc_y:
                    form_data.update({
                        ...
                        "metric": {"expressionType":"SQL","label":enc_y,"sqlExpression":f"SUM({enc_y})"},
                    })
                else:
                    form_data.update({
                        "groupby": group_by[:2],
                ...
                _logger.warning("viz_type '%s' belum di-handle khusus; gunakan form_data minimal", viz_type)

            # Axis/label opsional
            if axis.get("x_label"): form_data["x_axis_label"] = axis.get("x_label")
            if axis.get("y_label"): form_data["y_axis_label"] = axis.get("y_label")
            if isinstance(axis.get("rotate_x"), int): form_data["xAxisLabelRotation"] = axis.get("rotate_x")

            # Hints (debug, tidak dipakai Superset)
            form_data["_dataset_hint"] = {"schema": dataset_spec.get("schema"), "table": dataset_spec.get("table")}
            form_data["_encoding_hint"] = {"x": enc_x, "y": enc_y, "color": enc_color, "size": enc_size, "pivot": enc_pivot}
